...
description: dotfiles の設定ファイル（zsh / nvim / cmux / herdr / .Codex 等）を変更した後に環境を検査する。ユーザーが「環境チェックして」「doctorして」と言ったとき、または /dotfiles-doctor 実行時に使う。
---
...
- 新しいツールの設定を足した場合は、`setup.sh` の link 対象と `bin/dotfiles-doctor` の `config_targets` / `tools` への追加も検査対象（AGENTS.md の規約）
